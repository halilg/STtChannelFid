// https://github.com/halilg/cmssw/blob/CMSSW_7_0_X/DataFormats/Math/interface/deltaPhi.h
...
namespace mymath {
  ...
  inline double delta_Phi(double phi1, double phi2) { 
    double result = fabs(fabs(fabs(phi1-phi2) -  M_PI) - M_PI);
    return result;
  }

  inline float delta_Phi(float phi1, float phi2) { 
    float result = fabs(fabs(fabs(phi1-phi2) -  M_PI) - M_PI);
    return result;
  }
...
// lovely!  VI
using mymath::deltaPhi;

template<typename T1, typename T2 = T1>
struct DeltaPhi {
  double operator()(const T1 & t1, const T2 & t2) const {
    return mymath::deltaPhi(t1, t2);
  }
};
